...
def split_designators_text(text):
    """把 BOM 单元格中的位号规范化为列表。
    支持：R1,R2 / R1 R2 / R1-R5 / C1,C2；并过滤明显不是位号的内容。
    """
...
def _normalize_key_value(v):
    """主键归一化：去首尾空白、统一大小写、压缩内部空白。"""
    return re.sub(r"\s+", "", str(v or "").strip()).upper()


def _normalize_qty(v):
    """数量归一化：1、1.0、1,000 -> 同一个整数；非整数数量保留浮点。"""
...
def compare_excel_excel_mpn(path_a, path_b, key_a=None, key_b=None, qty_a=None, qty_b=None):
    ...
    def build(headers, data, mcol_list, qcol, refcol, mfgcol, itmcol, desccol):
        ...
        idx = {}
        for row in data:
            pk = tuple(_normalize_key_value(row["values"][i]) for i in mi)
            if not any(pk):
                continue
            rec = {
                "row": row,
    ...
    for k in keys:
        ...
        if not ra_:
            ...
            rows.append({"item": label, "key": k, "status": "仅文件B", "rec_a": None, "rec_b": rb_[0],
                         "field_diffs": [], "hdra": ha, "hdrb": hb})
            continue
        # 主键一致：先聚合重复行，再比较数量 / 位号 / 厂商 / 料号 / 描述
        def aggregate(records):
            qty_values = [_normalize_qty(r["qty"]) for r in records]
        ...
        qa_n, qb_n = _normalize_qty(ra0["qty"]), _normalize_qty(rb0["qty"])
        if qa_n != qb_n:
            diffs.append({"field": "数量(%s)" % (qa or "Quantity"),
                          "a": str(ra0["qty"] or ""), "b": str(rb0["qty"] or ""),
                          "raw_a": str(ra0["qty"] or ""), "raw_b": str(rb0["qty"] or "")})
        ...
        if _normalize_text(mfa_) != _normalize_text(mfb_):
            diffs.append({"field": "厂商(Manufacturer)", "a": mfa_, "b": mfb_,
                          "raw_a": mfa_, "raw_b": mfb_})

        if _normalize_text(ra0["itm"]) != _normalize_text(rb0["itm"]):
            diffs.append({"field": "内部料号(Item Number)", "a": str(ra0["itm"] or ""),
                          "b": str(rb0["itm"] or ""), "raw_a": ra0["itm"], "raw_b": rb0["itm"]})

        if _normalize_text(ra0["desc"]) != _normalize_text(rb0["desc"]):
            diffs.append({"field": "描述(Description)", "a": str(ra0["desc"] or ""),
                          "b": str(rb0["desc"] or ""), "raw_a": ra0["desc"], "raw_b": rb0["desc"]})
        if diffs:
            n_diff += 1
...
def run_gui():
    ...
    root = tk.Tk()
    root.title("BOM ↔ PDF 原理图 器件核对工具 V2")
...
def _start_progress(app, root, text):
    ...
    def _run():
        ...
        root.title(f"⏳ {text} — BOM ↔ PDF 器件核对工具 V2")
    root.after(0, _run)
...
def run_cli(bom_path=None, pdf_path=None, col_name="Part Reference", mode="位号核对(推荐)",
    ...
    for b in bad_list:
        print("      MISS  %s  (BOM第%s行)" % (b["designator"], b.get("row", "")))
    print("[4/4] 生成报告...")
    if not out_dir:
        out_dir = os.path.dirname(os.path.abspath(bom_path))
